...
def train(T=500, cfg=True, img_size=16, input_channels=3, channels=32, 
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i, (images, labels) in enumerate(pbar):
            ...
            if np.random.rand() < p_uncod: # Discard every 1/10 labels
                labels = None

            t = torch.randint(0, T, (images.shape[0],), device=device) # Generate random time steps for each image sampled between 0 and T

            x_t, noise = diffusion.q_sample(images, t)
            predicted_noise = model(x_t, t, labels)
            loss = mse(predicted_noise, noise) #MSE ERROR
